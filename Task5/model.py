...
class LSTM(nn.Module):
    ...
    def forward(self, x, lens, hidden):
        ...
        :return: (batch, seq_len, hidden_size), tuple(h, c)
        """
        # Compress a filled variable - length sequence
        packed_x = nn.utils.rnn.pack_padded_sequence(x, lens, batch_first=True)
        packed_output, (h, c) = self.lstm(packed_x, hidden)
        # Fill in the compressed sequence again
...
class LSTM_LM(nn.Module):
    ...
    def forward(self, x, lens, hidden):
        embed = self.embed(x)
        hidden, (h, c) = self.lstm(self.dropout(embed), lens, hidden)
